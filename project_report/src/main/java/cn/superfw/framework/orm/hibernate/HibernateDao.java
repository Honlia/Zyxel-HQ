 ...
 *
 * $Id: HibernateDao.java 1205 2010-09-09 15:12:17Z superch $
 */
package cn.superfw.framework.orm.hibernate;
...
import cn.superfw.framework.orm.PropertyFilter;
import cn.superfw.framework.orm.PropertyFilter.MatchType;
import cn.superfw.framework.utils.ReflectionUtils;
...
public class HibernateDao<T, PK extends Serializable> extends BaseHibernateDao<T, PK> {
	 ...
	 *
	 * @param page 分页参数.
	 * @param associationPath 关联表
	 * @param criterions 数量可变的Criterion.
	 * @param associationCriterions 关联表数量可变的Criterion.
	 * @return 分页查询结果.附带结果列表及所有查询输入参数.
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
    public Page<T> findAssociationPage(final Page<T> page, final String associationPath, final Criterion[] criterions, final Criterion[] associationCriterions) {
        Assert.notNull(page, "page不能为空");

        Criteria c = createCriteria(criterions);
        c.createCriteria(associationPath, associationPath);
        for (Criterion associationCriterion : associationCriterions) {
            c.add(associationCriterion);
        }
